        ...
        public void RequestFileHandler()
        {
            try
            {
                Task.Run( async() => {
                    ...
                    int queueLengh = Convert.ToInt32(builder.ToString());
                    byte[] NEXT = Encoding.Unicode.GetBytes("Next");
                    byte[] LAST = Encoding.Unicode.GetBytes("Last");
                    while (queueLengh > 0 && client.Connected)
                    {
